...
bus %>%
  ...
  geom_histogram() +
  scale_fill_manual(name = 'Restaurant Open Status', labels = c("Closed", "Open"), values = c("#FBC1C1", "#EA7070")) +
  theme_minimal() +
...
bus %>%
  ggplot() +
    aes(x = as.character(is_open), y = review_count) +
  # Summarize data, fun.data is a function that returns ymin, y, ymax
    stat_summary(fun.data="mean_sdl", geom="errorbar") +
    stat_summary(fun.data="mean_sdl") + 
    xlab("Restaurant Open Status") +
    ylab("FNumber of Reviews") +
    labs(title="Box Plot for Review Count by Open Status") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme_minimal()

bus %>%
  ggplot(aes(x=as.character(is_open), y=review_count)) +
...
bus %>%
  ...
  xlab("Price Range") + 
  ylab("Frequency") +
  labs(title="Bar Chart of Restaurant Rating by Open Status") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
# Price_range
bus %>%
  ggplot(aes(x=price_range, fill = as.character(is_open))) +
  geom_bar() +
    scale_fill_manual(name = 'Restaurant Open Status', labels = c("Closed", "Open"), values = c("#FBC1C1", "#EA7070")) +
  theme_minimal() +
  xlab("Price Range") + 
  ylab("Frequency") +
  labs(title="Bar Chart of Restaurant Price Range by Open Status") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
# Takeout & Rating & is_open

# Avg_hours $ Rating

# Price range & Rating

# Review count & Rating & is_open
