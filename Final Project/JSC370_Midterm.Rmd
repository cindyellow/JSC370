...
for (hours in bus$hours){
  ...
  for (day in days){
    ...
    open <- as.integer(str_remove(str_extract(day, "[0-9]+(:)"), "[:]"))
    close <- as.integer(str_remove(str_extract(day, "(-)[0-9]+"), "[-]"))
    for (i in range(length(close))){
      if(is.na(close[i]) | is.na(open[i])){
        total_length <- NA
        break
      }
      if (close[i] > open[i]){
        length <- sum(length, close[i] - open[i])
      } else {
        length <- sum(length, (close[i] + 24) - open[i])
      }
      total_length <- sum(total_length, length)
    }
    
  }
  avg_length <- total_length/num_days
  avg_hours <- c(avg_hours, avg_length)
}
...
# for (bus in bus$attributes){
#   all <- str_remove_all(bus, "[{}]")
#   attr <- strsplit(all, ", ")
#   # Find takeout
#   for (i in range(length(attr))){
#     if ("RestaurantsTakeOut" %in% attr[i]){
#       val <- str_remove(str_extract(day, "'[TF][a-z]+'"), "[']")
#       bus[, takeout := fifelse(val == 'True', 1, 0)]
#     }
#   }
# }
