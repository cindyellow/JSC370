...
Using microbenchmark to compare the runtime of the variation of the two functions, we find that `fun1` on average takes`r 100 * test1_df$mean[1]/test1_df$mean[2]`% more time than `fun1alt`. For function 2, `fun2` is on average `r 100 * test2_df$mean[1]/test2_df$mean[2]`% slower than `fun2alt`. Below is a full table of the runtime performance comparison with the unit microseconds.
...
By using parallel computing, we have reduced runtime by `r (2.1 - 1.89)*100/2.1`%. 

# ML

In the following steps, we will be examining the `hitters` dataset, which details information for 332 major league baseball players. We will compare the models' performance for predicting players' salaries based on provided features. NA observations are ommitted because we won't be able to calculate MSEs otherwise.
...
First, we fit a regression tree with the response variable `Salary` using the ANOVA method. The tree below shows the predicted value for eacch node as well as the percentage of the observations in the node. 
...
For this tree, the optimal CP value is `optimalcp`.
...
From the graph of $\lambda$ versus MSE, we see that the best value is $\lambda = 1$. Using the model with the best shrinkage parameter on the training set, we construct its variable importance table and plot. 
...
```
For XGBoost, we again see `CRuns`, `CRBI`, and `CHits` reappear. Although `HmRun` is not among the top 5 here, `CHmRun` is, indicating the importance of home runs in predicting the player's salary. It is interesting to note that `Walks` and even `CWalks` are not considered very important in XGBoost. 
