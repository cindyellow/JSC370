...
medians <- station_ave[, .(temp_50 = quantile(temp, probs = .5, na.rm = T),
                           ...
                           atm.press_50 = quantile(atm.press, na.rm = T))]
```
...
# Median wind.sp
station_ave[, windsp_dist := abs(wind.sp - medians$wind.sp_50)]
median_windsp_station <- station_ave[order(windsp_dist)][1]

# Median atm.press
station_ave[, atm_dist := abs(atm.press - medians$atm.press_50)]
median_atm_station <- station_ave[order(atm_dist)][1]
```

- No, the three stations are different. In terms of temperature, the station is located in Kentucky, whereas the one for median windspeed is in , and the one for atmosphere pressure is in Georgia.
...
```{r}
med <- station_ave[, .(temp_50 = quantile(temp, probs = .5, na.rm = T),
                           wind.sp_50 = quantile(wind.sp, probs = .5, na.rm = T),
                           atm.press_50 = quantile(atm.press, na.rm = T)),
                       by = STATE]

med_eu <- merge(x=met, y=med, by="STATE")

med_eu[, temp_eudist := sqrt((temp - medians$temp_50)^2 + (wind.sp - medians$wind.sp_50)^2 + (atm.press - medians$atm.press_50)^2)]

eu_station <- med_eu[, .SD[which.min(temp_eudist)], by=STATE]
```
...
leaflet() %>%
  ...
  addCircles(data = eu_station,
             lat =~ lat, lng =~lon, popup="median station",
             opacity = 1, fillOpacity = 1, radius = 400, color = "Orange")
```
...
knitr::kable(tab, caption = "A summary of data by temperature category")
```
...
plot(gam_mod)
```
In terms of adjusted R squared, the spline model with 20 knots is better with a value of 0.67 compared to 0.017 for the linear model. If we don't consider the possibility of overfitting and simply look at its performance on the current dataset, the spline model has better fit for the increased variation in data that it explains. Regardless of overfitting, spline is more suitable as it can pick up the non-linear patterns in the dataset visible in the first plot.
...
https://github.com/cindyellow/JSC370-labs/tree/master
